...
def run_assessment(window: InputWindow, config: AssessmentConfig | None = None) -> MissionAssessment:
    ...
    return MissionAssessment(
        ...
        input_manifest={"scenario_id": window.scenario_id, "source": "data/fixtures/scenarios.json", "fixture_version": "0.1.0"},
        quality_findings=findings,
        orbit=orbit,
        telemetry=telemetry,
        decision=decision,
        versions={"package": "0.1.0", "policy": config.policy_version, "orbit_model": config.orbit_model_version, "telemetry_model": config.telemetry_model_version},
        limitations=[
